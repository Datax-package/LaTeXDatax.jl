using Latexify, Unitful, LaTeXStrings
import Unitful: Unit,FreeUnits,Quantity,power,abbr,name,tens,sortexp
...
@latexrecipe function f(p::T;unitformat=:mathrm) where T <: Unit
    ...
    unitname = (unitformat == :mathrm ?
                abbr(p) :
                "\\$(lowercase(String(name(p))))")
    pow = power(p)
